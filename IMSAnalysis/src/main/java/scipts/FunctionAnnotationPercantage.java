...
public class FunctionAnnotationPercantage {
	...
	public static void main(String[] args) {
		try {
			...
			String folder = "data/sihumi_db1/smartLCA/function/";
			File dataFolder = new File(folder);
			Iterator<Path> files = Files.list(dataFolder.toPath()).iterator();
			BufferedWriter bw = new BufferedWriter(new FileWriter(new File(folder + "percantageAnalysisF.tsv")));
			while (files.hasNext()) {
				Path aFile = files.next();
				if (aFile.getFileName().toString().equals("percantageAnalysisF.tsv")) {
					continue;
				}
				...
				br.close();
				for (String level : known.keySet()) {
					bw.write(level + "\t" + known.get(level).toString() + "\t" + unknown.get(level).toString() + "\n");
				}
			}
			bw.close();
			...
			// GUT
			// read all files, extract the unclassified, write a new file for plots  
			folder ="data/gut_db2/smartLCA/function/";
			dataFolder = new File(folder);
			bw = new BufferedWriter(new FileWriter(new File(folder + "percantageAnalysisF.tsv")));
